...
{
  Real uh = sqrt(_basic_def_old[_qp](1, 0) * _basic_def_old[_qp](1, 0) + _basic_def_old[_qp](2, 0) * _basic_def_old[_qp](2, 0));
  ...
  // Updating current value of critical Buckling load bassed on previous step horizontal displacements
  ...
  if (_basic_def[_qp](0, 0) > _ucrn)
  {
    if (_cavitation) // simulating cavitation effects
    {
      ...
      else if (_basic_def[_qp](0, 0) <= _umax_old[_qp](0))
      {
        ...
        _Fb[_qp](0, 0) = _Fcn + (_Fmax - _Fcn) / (_umax[_qp](0)- _ucn[_qp](0)) * (_basic_def[_qp](0, 0) - _ucn[_qp](0));
      }
...
ComputeLRIsolatorElasticity::computeShear()
{
  ...
  // Real accl1 = _basic_accel_old[_qp](1,0)+(_basic_def[_qp](1, 0)-_basic_def_old[_qp](1,0))/(_beta*_dt*_dt)-(_basic_vel_old[_qp](1,0))/(_beta*_dt)-(_basic_accel_old[_qp](1,0))/(2*_beta);
  // Real accl2 = _basic_accel_old[_qp](2,0)+(_basic_def[_qp](2, 0)-_basic_def_old[_qp](2,0))/(_beta*_dt*_dt)-(_basic_vel_old[_qp](2,0))/(_beta*_dt)-(_basic_accel_old[_qp](2,0))/(2*_beta);
  ...
  if (sqrt(pow(delta_ub(1), 2) + pow(delta_ub(2), 2)) >= 0.0)
  {
      ...
      do
      {
        ...
        Df(0,0) = 1.0 + (1.0/uy) * (2 * _z[_qp](0) * delta_ub(1) * tmp1 + _z[_qp](1) * delta_ub(2) * tmp2);
        Df(1,0) = (tmp1 / uy) * _z[_qp](1) * delta_ub(1);
        Df(0,1) = (tmp2 / uy) * _z[_qp](0) * delta_ub(2);
        Df(1,1) = 1.0 + (1.0 / uy) * (_z[_qp](0) * delta_ub(1) * tmp1 + 2 *_z[_qp](1)* delta_ub(2) * tmp2);
      ...
      _dzdu(0,0) = (1.0/uy)*(1.0 - _z[_qp](0)*(_z[_qp](0)*tmp1+_z[_qp](1)*tmp2*du2du1));
      _dzdu(0,1) = (1.0/uy)*(du1du2-_z[_qp](0)*(_z[_qp](0)*tmp1*du1du2+_z[_qp](1)*tmp2));
      _dzdu(1,0) = (1.0/uy)*(du2du1-_z[_qp](1)*(_z[_qp](0)*tmp1+_z[_qp](1)*tmp2*du2du1));
      _dzdu(1,1) = (1.0/uy)*(1.0 - _z[_qp](1)*(_z[_qp](0)*tmp1*du1du2+_z[_qp](1)*tmp2));
